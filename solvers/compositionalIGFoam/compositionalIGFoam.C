...
License
    This solver developed by Ali Papi (Ali.Papi@hw.ac.uk) is an extension to porousMultiphaseFoam, an extension of OpenFOAM
...
Description
    Transient solver for incompressible two-phase flow (Darcy's law) with species transport and
		mass transfer between the phases in porous media
    using the IMPES method (IMplicit Pressure Explicit Saturation)
		by assuming equilibrium between the phases with Henry's and Raoult's laws.
